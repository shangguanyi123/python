...
import time
import pytest
from website_api.ichempro_api import Chanpinguanli, Xitongguanli
...
# 产品分类
def test_prod_class_a(product_data):
    print(
    ...
    product_data.update({'一级测试产品分类id': res2.json()['data']['data'][0]['id']})
    # 修改一级测试产品分类
    res6 = Chanpinguanli().update_chanpinfenlei(0, product_data['一级测试产品分类id'], int(time.time()),
                                                '一级测试产品分类test', 0, 1)
    ...
    product_data.update({'一级测试产品分类id': res9.json()['data']['data'][0]['id']})
    # 新增二级测试产品分类
    res3 = Chanpinguanli().add_chanpinfenlei(1, int(time.time()), '二级测试产品分类', 1, 1,
                                             product_data['一级测试产品分类id'])
    ...
    product_data.update({'二级测试产品分类id': res4.json()['data']['data'][0]['id']})
    # 修改产品分类
    res7 = Chanpinguanli().update_chanpinfenlei(product_data['一级测试产品分类id'], product_data['二级测试产品分类id'],
                                                int(time.time()),
                                                '二级测试产品分类test', 0, 1)
    ...
    product_data.update({'二级测试产品分类id': res10.json()['data']['data'][0]['id']})
    # 删除二级产品分类
    res5 = Chanpinguanli().del_chanpinfenlei(product_data['二级测试产品分类id'])
    print('删除二级测试产品分类', res5.json())
    Assertion(res5).tongyong()
    # 删除一级产品分类
    res11 = Chanpinguanli().del_chanpinfenlei(product_data['一级测试产品分类id'])
...
def test_prod_class_b(product_data):
    print(
    ...
    print('查询二级测试产品分类', res4.json())
    Assertion(res4).chapinfenlei_chaxun(1, '二级测试产品分类')
    product_data.update({'二级产测试产品分类id': res4.json()['data']['data'][0]['id']})
    # 删除一级产品分类
    res5 = Chanpinguanli().del_chanpinfenlei(product_data['一级测试产品分类id'])
    print('删除一级测试产品分类', res5.json())
    assert res5.json()['message'] == '当前分类下有子分类，不可删除'
    # 删除二级产品分类
    res6 = Chanpinguanli().del_chanpinfenlei(product_data['二级产测试产品分类id'])
    print('删除二级产品分类', res6.json())
    Assertion(res6).tongyong()
    # 删除一级产品分类
    res7 = Chanpinguanli().del_chanpinfenlei(product_data['一级测试产品分类id'])
    ...
    Assertion(res8).chaxun_isnull()
# 产品品牌
def test_brand(product_data):
    print('新增品牌-查询品牌-修改品牌-查询品牌-删除品牌-查询品牌')
    ...
    product_data.update({'品牌id': res2.json()['data']['data'][0]['id']})
    res3 = Chanpinguanli().update_pinpai(product_data['品牌id'], '测试品牌test', 4.5, 7.5, 1)
    print('修改品牌', res3.json())
    Assertion(res3).tongyong()
    res4 = Chanpinguanli().sel_pinpai('测试品牌test')
    print('查询', res4.json())
    Assertion(res4).pinpai_chaxun('测试品牌test')
    res5 = Chanpinguanli().del_pinpai(product_data['品牌id'])
    print('删除品牌', res5.json())
    Assertion(res5).tongyong()
    res6 = Chanpinguanli().sel_pinpai('测试品牌test')
    Assertion(res6).chaxun_isnull()
# cas列表
def test_cas_list(product_data):
...
def test_add_prod(product_data):
    ...
    product_data['包装id'] = res7.json()['data']['data'][0]['id']
    product_data['包装名称'] = res7.json()['data']['data'][0]['name']
    res8 = Xitongguanli().sel_danwei('measure')
    print('获取计量单位', res8.json())
    product_data['计量id'] = res8.json()['data']['data'][0]['id']
    product_data['计量名称'] = res8.json()['data']['data'][0]['name']
    ...
    product_data['图片']['id'] = res9.json()['data'][0]['id']
    product_data['图片']['filename'] = res9.json()['data'][0]['filename']
    product_data['图片']['path_url'] = res9.json()['data'][0]['path_url']
    #新增一级产品分类
    ...
    product_data['一级测试产品分类id'] = res12.json()['data']['data'][0]['id']
    # 新增品牌
    ...
    product_data['品牌id'] = res14.json()['data']['data'][0]['id']
    product_data['品牌名称'] = res14.json()['data']['data'][0]['name']
    # 辅助属性取值
    for i in range(0, len(res4.json()['data']['prods'])):
        if res4.json()['data']['prods'][i]['enum_key']:
            ...
            product_data['辅助属性'][f'{field_name}'] = str(random.randint(1, len(enum_val)))
        else:
            field_name = res4.json()['data']['prods'][i]['field_name']
            product_data['辅助属性'][f'{field_name}'] = 'test'
    print(product_data)
    res10 = Chanpinguanli().add_chanpin(
        ...
        'raw_material',
        '冷藏',
        product_data['危险品标识id'],
        product_data['辅助属性'],
        product_data['图片']['id'],
        product_data['图片']['filename'],
        product_data['图片']['path_url'],
        product_data['产品编号'] + '-10kg',
        '98%',
        product_data['品牌名称'],
        product_data['品牌id'],
        product_data['包装id'],
    ...
    product_data = {**product_data, **new_values}
    print(product_data)
    res17 = Chanpinguanli().cas_shangchuan_mol(r'C:\Users\shangguanyi\Downloads\1189169-37-6.mol')
    print('上传mol',res17.json())
    product_data['mol']['id'] = res17.json()['data']['structure_img']['id']
    product_data['mol']['filename'] = res17.json()['data']['structure_img']['filename']
    product_data['mol']['path_url'] = res17.json()['data']['structure_img']['path_url']
    res18 = Chanpinguanli().sel_pinpai()
    print('查询品牌',res18.json())
    index = random.randint(0, len(res18.json()['data']['data']) - 1)
    product_data['品牌id2'] = res18.json()['data']['data'][index]['id']
    product_data['品牌名称2'] = res18.json()['data']['data'][index]['name']
    res19 = Xitongguanli().sel_danwei('package')
    print('查询包装单位',res19.json())
    index = random.randint(0,len(res19.json()['data']['data'])-1)
    product_data['包装id'] = res19.json()['data']['data'][index]['id']
    product_data['包装名称'] = res19.json()['data']['data'][index]['name']
    res20 = Xitongguanli().sel_danwei('measure')
    print('查询计量单位',res20.json())
    index = random.randint(0, len(res20.json()['data']['data']) - 1)
    product_data['计量id'] = res20.json()['data']['data'][index]['id']
    product_data['计量名称'] = res20.json()['data']['data'][index]['name']
    # 修改产品
    res16 = Chanpinguanli().update_chanpin(
        product_data['产品']['产品id'],
        product_data['产品']['产品编号'],
        product_data['产品']['cas'],
        product_data['产品']['产品名称'],
        product_data['产品']['产品分类'],
        product_data['产品']['产品类型'],
        product_data['产品']['储存条件'],
        product_data['产品']['危险品标签id'],
        1,
        product_data['产品']['产品属性'],
        product_data['产品']['图片']['id'],
        product_data['产品']['图片']['名称'],
        product_data['产品']['图片']['url'],
        product_data['mol']['id'],
        product_data['mol']['filename'],
        product_data['mol']['path_url'],
        product_data['产品']['规格id'],
        product_data['产品']['规格编号'],
        product_data['产品']['纯度'],
        product_data['产品']['品牌id'],
        product_data['产品']['品牌名称'],
        product_data['产品']['包装id'],
        product_data['产品']['包装名称'],
        product_data['产品']['单位转换'],
        product_data['产品']['计量id'],
        product_data['产品']['计量名称'],
        product_data['产品']['包装'],
        product_data['产品']['价格'],
        product_data['产品']['货期'],
        product_data['产品']['产品编号'] + '-100' + product_data['计量名称'],
        '88%',
        product_data['品牌id2'],
        product_data['品牌名称2'],
        product_data['包装id'],
        ...
        product_data['计量id'],
        product_data['计量名称'],
        '100' + product_data["计量名称"],
        '88.88',
        '现货'
    )
    print('修改产品',res16.json())
    #删除产品
    res20 = Chanpinguanli().del_chanpin(product_data['产品']['产品id'])
    print('删除产品',res20.json())
    #查询
    res21 = Chanpinguanli().sel_chanpin('',product_data['产品编号'])
    Assertion(res21).chaxun_isnull()
    #删除品牌、分类
    res22 = Chanpinguanli().del_pinpai(product_data['品牌id'])
    print('删除品牌',res22.json())
    Assertion(res22).tongyong()
    res23 = Chanpinguanli().del_chanpinfenlei(product_data['一级测试产品分类id'])
    print('删除产品分类',res23.json())
    Assertion(res23).tongyong()

def test_100_add_prod(product_data):
    for i in open(r'C:\Users\shangguanyi\Desktop\products.txt', 'r', encoding='utf-8'):
        ...
        product_data['品牌id'] = res14.json()['data']['data'][index]['id']
        product_data['品牌名称'] = res14.json()['data']['data'][index]['name']
        # 辅助属性取值
        for i in range(0, len(res4.json()['data']['prods'])):
            if res4.json()['data']['prods'][i]['enum_key']:
                field_name = res4.json()['data']['prods'][i]['field_name']
